...
> 一个面向安全研究与教学的 Java 反序列化漏洞靶场平台。基于 LLM + MCP 自动分析 `java-chains.jar` 中的 Gadget Chain，为每条链生成**仅包含必需依赖**的最小化 Spring Boot 漏洞环境，并通过 Docker Compose 一键编排 51 个独立靶标容器与统一网关。
...
1. 利用 LLM 解析 `java-chains.jar`（来自 `Y4er/java-chains`）中内置的 51 条 Gadget Chain 配方；
...
├── docker-compose.yml              # 一键拉起 gateway + 51 个链容器
│
...
│   └── test_config.yaml
│
├── gadget-dependencies/            # 51 条链的 pom.xml（LLM 生成产物）
│   ├── gadget_chains_summary.json
│   ├── CB1链/pom.xml
│   ├── K1链/pom.xml
│   └── ... (共 51 个)
│
├── generated-env/                  # 51 个最小化靶标 Spring Boot 工程
...
首次构建约 30–60 分钟（51 个镜像并行受 Docker 资源限制，可能需要分批）。完成后查看状态：

```bash
docker-compose ps
```

预期输出：`deserialization-gateway` 与 51 个 `xxx-env` 容器全部 `Up`。
...
**Q1：`docker-compose up` 时报端口冲突？**
A：51 个链容器占用 `8081–8131` 端口，请确保这些端口未被占用，或修改 `docker-compose.yml` 中对应映射。

**Q2：构建过慢或卡死？**
A：Maven 首次下载依赖较慢，建议配置国内镜像（阿里云）。可设置环境变量 `MAVEN_OPTS=-Dmaven.repo.snapshot=false`。

**Q3：`gateway` 容器读取不到链信息？**
