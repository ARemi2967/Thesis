...
| `generated-env/` | 每条链对应的可独立构建的 Spring Boot 靶标工程 | `8081–` |
...
│   └── test_config.yaml
│
├── gadget-dependencies/            #  pom.xml（LLM 生成产物）
...
│   └── ... (从 8081 端口开始)
│
├── java-chains/                    # 上游 gadget chain 数据源（需自行下载 v1.4.4）
│   ├── java-chains.jar              # 不在仓库内，用户需自行下载
│   └── chains-config/
│
└── test-reports/
...
| Git | 任意 | 克隆仓库 |
| **java-chains** | **1.4.4** | **需自行下载，见下方说明** |

内存建议 ≥ 16 GB，磁盘空间 ≥ 30 GB（构建产物较多）。

### 获取 `java-chains` v1.4.4

由于上游授权限制，本仓库**不打包** `java-chains.jar`，请按以下步骤自行获取并放入项目根目录：

1. 访问上游发布页：<https://github.com/Y4er/java-chains/releases>
2. 下载 **`java-chains-1.4.4`** 版本（ Release 资产中的 `java-chains-1.4.4.jar` 或同版本发行包）
3. 重命名为 `java-chains.jar` 并放置于：

   ```
   <项目根>/java-chains/java-chains.jar
   ```

   最终路径应与下文示例一致：

   ```bash
   ls java-chains/java-chains.jar
   # java-chains/java-chains.jar
   ```
...
### 2. 放入 `java-chains` v1.4.4

仓库不含 `java-chains.jar`，需自行下载 1.4.4 版本（[环境要求](#环境要求) 一节已有说明）：

```bash
mkdir -p java-chains
# 从 https://github.com/Y4er/java-chains/releases 下载 1.4.4 版本
cp /path/to/java-chains-1.4.4.jar java-chains/java-chains.jar
```
...
首次构建约 30–60 分钟（全部镜像并行受 Docker 资源限制，可能需要分批）。完成后查看状态：
